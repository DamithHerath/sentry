...
class OrganizationMemberTest(TestCase):
    ...
    def test_send_request_notification_email(self):
        ...
        assert mail.outbox[0].to == ["owner@localhost"]
        assert mail.outbox[1].to == ["manager@localhost"]

        expected_subject = "Member has requested to invite a member to %s on Sentry" % (
            organization.name,
        )
